...
def user_input():
    while True:
        ...
        time.sleep(1)
        print("\nRemember that scanning a target may seem harmless, but can be intrusive. \nRemember to seek prior authorisation when conducting scans \nwith Piñata.\nIn order for the scanner to run you need to make sure that \nsocket, scapy, sys, and asyncio are installed.")
        print("\nTo install the necessary libraries, exit the scanner and type \"pip install [whatever library you're missing]\" in the command line interface\n")
        time.sleep(1)
